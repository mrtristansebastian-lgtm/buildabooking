...
export const FinancePaymentSettings = ({ appId, businessId, canManageWorkspace, showToast, bookings = [], onMarkBookingPaid, showExamples = false }) => {
  ...
  const visibleDeskRows = useMemo(() => {
    const rows = financeRecords.length ? financeRecords : (showExamples ? exampleTransactions : []);
  ...
  const downloadFinanceCsv = () => {
    const rows = (financeRecords.length ? financeRecords : (showExamples ? exampleTransactions : [])).map((row) => ({
      id: row.id,
      status: row.status,
