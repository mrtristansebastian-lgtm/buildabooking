...
export const BookingSuccessState = ({
    ...
    return (
    <div className={`booking-success-step min-h-full flex items-center justify-center ${previewSuccessMotionClass} p-4 md:p-10 relative z-10`}>
        <main className="booking-success-panel flex w-full max-w-3xl flex-col rounded-3xl border bg-white p-5 md:p-7 text-left" style={{ borderColor: `${settings.headingColor || '#000000'}12` }}>
            <div className={`booking-success-hero flex items-start justify-between gap-5 ${inspectClass}`} onClick={() => previewInspectEnabled && onInspect('buttons')}>
                <div className="min-w-0">
                    <p className="text-[10px] font-black uppercase tracking-[0.34em] opacity-35" style={{ color: settings.bodyColor }}>Booking Status</p>
                    ...
                    <p className="booking-success-copy mt-3 max-w-xl text-sm md:text-base font-semibold leading-relaxed opacity-55" style={{ color: settings.bodyColor, ...(subtextLetterSpacing ? { letterSpacing: subtextLetterSpacing } : {}) }}>
                        {isWaitlistMode ? `You are on the standby list for ${activeDate.month} ${activeDate.dayNum}. The business can contact you if a slot opens.` : `Your request for ${selectedTime} on ${activeDate.dayNum} ${activeDate.month} is saved for the business to review.`}
                    </p>
                </div>
            ...
            <div className="booking-success-details mt-6 grid gap-3 md:grid-cols-[1fr_1fr]">
                <div className="booking-success-detail rounded-2xl border p-4" style={{ borderColor: `${settings.headingColor || '#000000'}10`, backgroundColor: `${settings.headingColor || '#000000'}04` }}>
                    <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.24em]" style={{ color: settings.headingColor }}>
                        <span className="h-1.5 w-1.5 rounded-full bg-current" /> Reference
                    </p>
                    <p className="mt-2 text-xl font-black tracking-tight" style={{ color: settings.headingColor }}>{paymentReference}</p>
                    <p className="mt-1 text-xs font-semibold opacity-50" style={{ color: settings.bodyColor }}>Keep this for updates with the business.</p>
                </div>
                <div className="booking-success-detail rounded-2xl border p-4" style={{ borderColor: `${settings.headingColor || '#000000'}10`, backgroundColor: `${settings.headingColor || '#000000'}04` }}>
                    <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.24em]" style={{ color: settings.headingColor }}>
                        <span className="h-1.5 w-1.5 rounded-full bg-current" /> Next
                    </p>
                    <p className="mt-2 text-sm font-black" style={{ color: settings.headingColor }}>{isWaitlistMode ? 'Waitlist review' : 'Business review'}</p>
                    <p className="mt-1 text-xs font-semibold leading-relaxed opacity-50" style={{ color: settings.bodyColor }}>They can confirm, follow up, or help adjust the booking.</p>
                </div>
            </div>
